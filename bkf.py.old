...
class Conversation():
    ...
    def simple_delta(self, conversation:'Conversation')->'Conversation':
        ...
        delta  :List[Message] = conversation.history[i_self:]
        logging.info(f'n_delta: {len(delta)}')
        return Conversation(
                system_prompt=self.system_prompt,
                instructions =self.instructions,
                history      =delta,
                ct_tool      =self.ct_tool,
...
def get_llm_response_structured_plan(
    ...
    logging.info(f'get_llm_response_structured_plan('
        ...
        f'encoding={encoding})')
    checkpoint:Conversation = conversation.deepcopy()
    # TODO modify user prompt vs override system prompt ?
    # FIXME do *something* to the prompt? or not necessary, since the LLM knows it must produce a StructuredPlan ?
    # TODO include functions as context ?
    return get_llm_response_pydantic(
            llm         =llm,
            conversation=checkpoint,
            output_type =StructuredPlan,
            user_prompt =user_prompt, # TODO
            ct_tool     =ct_tool,
            im_start    =im_start,
            im_end      =im_end,
            requested   =requested,
...
# TODO critique-wrapped structured planner

def get_llm_response_with_structured_plan(...)->...:
    plan :StructuredPlan    = get_llm_response_structured_plan(...) # TODO
    steps:List[ReActResult] = []
    for objective in plan.objectives:
        logging.info(f'objective: {objective}')
        result              = get_critique_wrapped_react(
                llm         =llm,
                conversation=conversation,
                funcs       =funcs,
