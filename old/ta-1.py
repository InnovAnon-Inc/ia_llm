...
#    result:str = fmt.format(turn=turn.value, message=message, im_start=im_start, im_end=im_end)
...
class Conversation():
    ...
    #def set_system_prompt(self, system_prompt:str|LazyStr|None, im_start:str|None=None, im_end:str|None=None)->None:
...
def get_llm_response(
    ...
    logging.info(f'get_llm_response(ct_tool={ct_tool}, im_start={im_start}, im_end={im_end}, requested={requested}, threshold={threshold}, encoding={encoding})')
...
def get_llm_response_grammatical(
    ...
    logging.info(f'get_llm_response_grammatical(ct_tool={ct_tool}, im_start={im_start}, im_end={im_end}, requested={requested}, threshold={threshold}, encoding={encoding})')
...
#        _param['type']    = getattr(param.annotation, '__name__', str(param.annotation))
#    #if param.default != inspect._empty: # TODO include if json serializable ?
...
def get_llm_response_function_call(
        ...
        threshold     :int=10,
        encoding      :str='utf-8',
)->FunctionCallResult|FunctionCallError|FunctionCallNone:
...
def main()->None:
    ...
    if True:
        ...
        response        :FunctionCall           = get_llm_response_function_call(
                llm         =llm,
                user_prompt =user_prompt,
                funcs       =[hello_world,],
                conversation=Conversation(im_start=im_start, im_end=im_end,),)
